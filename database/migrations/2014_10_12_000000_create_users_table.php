...
class CreateUsersTable extends Migration
{
    ...
    public function up()
    {
        Schema::create('users', function (Blueprint $table) {
            ...
            // 口座情報
            $table->string     ('bank')        ->comment('銀行名')->nullable();
            $table->string     ('branch_code') ->comment('支店コード')->nullable();
            $table->tinyInteger('account_type')->comment('口座種別 0:普通, 1:当座')->nullable();
            $table->string     ('account_num') ->comment('口座番号')->nullable();
            $table->rememberToken();
            $table->timestamps();
        });
    }
